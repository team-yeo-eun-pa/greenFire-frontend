...
export const callReopenStoreAPI = (sellerCode) => {
    return async (dispatch) => {
        try {
            const result = await authRequest.put(`/seller/mystore/${sellerCode}/reopen`, {}, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
            console.log('callReopenStoreAPI result:', result);

            if (result?.status === 204) {
                dispatch(success());
                toast.success("스토어 운영이 재개되었습니다.");
            } else {
                toast.warning("스토어 운영 재개에 실패했습니다. 다시 시도해주세요.");
            }
        } catch (error) {
            ...
            if (error.response && error.response.data) {
                ...
                toast.error("스토어 운영 재개 중 오류가 발생했습니다. 다시 시도해주세요.");
            }
        }
    };
};
